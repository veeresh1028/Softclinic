...
namespace ClinicSoft.Areas.ConsentForms.Controllers
{
    public class StressTestController : Controller
    {
        ...
        // GET: ConsentForms/StressTest
        public ActionResult StressTestIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetStressTestData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.StressTest> list = BusinessLogicLayer.ConsentForms.StressTest.GetStressTestData(appId);
        ...
        public PartialViewResult CreateStressTest()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateStressTest");
                }
                else
                {
        ...
        public ActionResult InsertStressTest(BusinessEntities.ConsentForms.StressTest data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.StressTest.isValidStressTest(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.StressTest.SaveStressTest(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.stf_appId, "StressTest");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Stress Test Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateStressTest(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.StressTest ophtha = BusinessLogicLayer.ConsentForms.StressTest.GetStressTestData(appId).FirstOrDefault();
                    return PartialView("UpdateStressTest", ophtha);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditStressTest(BusinessEntities.ConsentForms.StressTest data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.StressTest.isValidStressTest(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.StressTest.SaveStressTest(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Stress Test Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteStressTest(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.StressTest.DeleteStressTest(appId, madeby);
        ...
        public PartialViewResult StressTestPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.StressTest.GetStressTestPreviousHistory(appId);
                    return PartialView("StressTestPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlStressTest(appId, emr, setId);
