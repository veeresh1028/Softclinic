...
{
    public class ThreadLiftController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/ThreadLift
        public ActionResult ThreadLiftIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetThreadLiftData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.ThreadLift> list = BusinessLogicLayer.ConsentForms.ThreadLift.GetThreadLiftData(appId);
        ...
        public PartialViewResult CreateThreadLift()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateThreadLift");
                }
                else
                {
        ...
        public ActionResult InsertThreadLift(BusinessEntities.ConsentForms.ThreadLift data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ThreadLift.isValidThreadLift(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.ThreadLift.SaveThreadLift(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.tlc_appId, "ThreadLift");

                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Thread Lift is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateThreadLift(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.ThreadLift thread = BusinessLogicLayer.ConsentForms.ThreadLift.GetThreadLiftData(appId).FirstOrDefault();

                    return PartialView("UpdateThreadLift", thread);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditThreadLift(BusinessEntities.ConsentForms.ThreadLift data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ThreadLift.isValidThreadLift(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.ThreadLift.SaveThreadLift(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Thread Lift Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteThreadLift(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.ThreadLift.DeleteThreadLift(appId, madeby);
        ...
        public PartialViewResult ThreadLiftPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.ThreadLift.GetThreadLiftPreviousHistory(appId);
                    return PartialView("ThreadLiftPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlThreadLift(appId, emr, setId);
