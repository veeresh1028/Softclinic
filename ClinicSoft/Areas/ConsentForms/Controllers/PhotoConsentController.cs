...
namespace ClinicSoft.Areas.ConsentForms.Controllers
{
    public class PhotoConsentController : Controller
    {
        ...
        // GET: ConsentForms/PhotoConsent
        public ActionResult PhotoConsentIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetPhotoConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.PhotoConsent> list = BusinessLogicLayer.ConsentForms.PhotoConsent.GetPhotoConsentData(appId);
        ...
        public PartialViewResult CreatePhotoConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreatePhotoConsent");
                }
                else
                {
        ...
        public ActionResult InsertPhotoConsent(BusinessEntities.ConsentForms.PhotoConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PhotoConsent.isValidPhotoConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.PhotoConsent.SavePhotoConsent(data, madeby);

                        if (isInserted > 0)
                        {

                            PDF _pdf = CreatePDF(data.cpcc_appId, "PhotoConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Photo Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdatePhotoConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.PhotoConsent photo = BusinessLogicLayer.ConsentForms.PhotoConsent.GetPhotoConsentData(appId).FirstOrDefault();

                    return PartialView("UpdatePhotoConsent", photo);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditPhotoConsent(BusinessEntities.ConsentForms.PhotoConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PhotoConsent.isValidPhotoConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.PhotoConsent.SavePhotoConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Photo Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeletePhotoConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.PhotoConsent.DeletePhotoConsent(appId, madeby);
        ...
        public PartialViewResult PhotoConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.PhotoConsent.GetPhotoConsentPreviousHistory(appId);
                    return PartialView("PhotoConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlPhotoConsent(appId, emr, setId);
