...
{
    public class OrthoElasticController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;
        // GET: ConsentForms/OrthoElastic
        public ActionResult OrthoElasticIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetOrthoElasticData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConcentForms.OrthoElastic> list = BusinessLogicLayer.ConcentForms.OrthoElastic.GetOrthoElasticData(appId);
        ...
        public PartialViewResult CreateOrthoElastic()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateOrthoElastic");
                }
                else
                {
        ...
        public ActionResult InsertOrthoElastic(BusinessEntities.ConcentForms.OrthoElastic data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.OrthoElastic.isValidOrthoElastic(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConcentForms.OrthoElastic.SaveOrthoElastic(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.oe_appId, "OrthoElastic");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Ortho Elastic is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateOrthoElastic(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConcentForms.OrthoElastic dental = BusinessLogicLayer.ConcentForms.OrthoElastic.GetOrthoElasticData(appId).FirstOrDefault();

                    return PartialView("UpdateOrthoElastic", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditOrthoElastic(BusinessEntities.ConcentForms.OrthoElastic data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.OrthoElastic.isValidOrthoElastic(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConcentForms.OrthoElastic.SaveOrthoElastic(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Ortho Elastic is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteOrthoElastic(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConcentForms.OrthoElastic.DeleteOrthoElastic(appId, madeby);
        ...
        public PartialViewResult OrthoElasticPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConcentForms.OrthoElastic.GetOrthoElasticPreviousHistory(appId);
                    return PartialView("OrthoElasticPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlOrthoElastic(appId, emr, setId);
