...
{
    public class DmaxCarbonController : Controller
    {
        int PageId = (int)Pages.DermatologyConsentForms;
        int PageId2 = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/DmaxCarbon
        public ActionResult DmaxCarbonIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetDmaxCarbonData(int appId)
        {
            int Action = (int)Actions.Read;
            if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
            {
                List<BusinessEntities.ConsentForms.DmaxCarbon> list = BusinessLogicLayer.ConsentForms.DmaxCarbon.GetDmaxCarbonData(appId);
        ...
        public PartialViewResult CreateDmaxCarbon()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    return PartialView("CreateDmaxCarbon");
                }
                else
                {
        ...
        public ActionResult InsertDmaxCarbon(BusinessEntities.ConsentForms.DmaxCarbon data)
        {
            int isInserted = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxCarbon.isValidDmaxCarbon(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DmaxCarbon.SaveDmaxCarbon(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cdc_appId, "DmaxCarbon");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Carbon Laser Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDmaxCarbon(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    BusinessEntities.ConsentForms.DmaxCarbon derma = BusinessLogicLayer.ConsentForms.DmaxCarbon.GetDmaxCarbonData(appId).FirstOrDefault();
                    return PartialView("UpdateDmaxCarbon", derma);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDmaxCarbon(BusinessEntities.ConsentForms.DmaxCarbon data)
        {
            int isEdited = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxCarbon.isValidDmaxCarbon(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DmaxCarbon.SaveDmaxCarbon(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Carbon Laser Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDmaxCarbon(int appId)
        {
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    int val = BusinessLogicLayer.ConsentForms.DmaxCarbon.DeleteDmaxCarbon(appId, madeby);
        ...
        public PartialViewResult DmaxCarbonPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DmaxCarbon.GetDmaxCarbonPreviousHistory(appId);
                    return PartialView("DmaxCarbonPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlDmaxCarbon(appId, emr, setId);
