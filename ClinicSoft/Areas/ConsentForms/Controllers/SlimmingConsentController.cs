...
{
    public class SlimmingConsentController : Controller
    {
        int PageId = (int)Pages.HijamaConsentForms;
        // GET: ConsentForms/SlimmingConsent
        public ActionResult SlimmingConsentIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetSlimmingConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.SlimmingConsent> list = BusinessLogicLayer.ConsentForms.SlimmingConsent.GetSlimmingConsentData(appId);
        ...
        public JsonResult GetSlimmingSheetDropdownData(string query)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<CommonDDL> list = BusinessLogicLayer.ConsentForms.SlimmingConsent.GetSlimmingSheetDropdownData(query);
        ...
        public PartialViewResult CreateSlimmingConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateSlimmingConsent");
                }
                else
                {
        ...
        [HttpPost]
        public ActionResult InsertSlimmingConsent(Slimming_Consent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/Slimming/");
                    ...
                    isInserted = BusinessLogicLayer.ConsentForms.SlimmingConsent.SaveSlimmingConsent(data, madeby);

                    if (isInserted > 0)
                    {

                        PDF _pdf = CreatePDF(int.Parse(emr.appId), "SlimmingConsent");
                        return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                    }
                    else
                    {
                        return Json(new { isInserted = false, isSuccess = true, message = "Slimming Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                    }
        ...
        public PartialViewResult UpdateSlimmingConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.SlimmingConsent slimming = BusinessLogicLayer.ConsentForms.SlimmingConsent.GetSlimmingConsentData(appId).FirstOrDefault();

                    return PartialView("UpdateSlimmingConsent", slimming);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public ActionResult EditSlimmingConsent(BusinessEntities.ConsentForms.Slimming_Consent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    isEdited = BusinessLogicLayer.ConsentForms.SlimmingConsent.SaveSlimmingConsent(data, madeby);
                    ...
                    {
                        return Json(new { isEdited = false, isSuccess = true, message = "Slimming Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                    }
        ...
        [HttpPost]
        public ActionResult DeleteSlimmingConsent(int sscId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.SlimmingConsent.DeleteSlimmingConsent(sscId, madeby);
        ...
        public PartialViewResult SlimmingConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.SlimmingConsent.GetSlimmingConsentPreviousHistory(appId);
                    return PartialView("SlimmingConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlSlimmingConsent(appId, emr, setId);
        ...
        public JsonResult GetAllSlimming(int appId, int? addeId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.SlimmingConsent> Slimming = BusinessLogicLayer.ConsentForms.SlimmingConsent.GetAllSlimming(appId, addeId);

                return Json(Slimming, JsonRequestBehavior.AllowGet);
            }
            else
            {
                return Json(new { isAuthorized = false, message = "UnAuthorised Access!" }, JsonRequestBehavior.AllowGet);
