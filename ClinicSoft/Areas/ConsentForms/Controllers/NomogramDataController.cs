...
{
    public class NomogramDataController : Controller
    {
        int PageId = (int)Pages.OphthalmologyConsentForms;
        // GET: ConsentForms/NomogramData
        public ActionResult NomogramDataIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetNomogramDataData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.NomogramData> list = BusinessLogicLayer.ConsentForms.NomogramData.GetNomogramDataData(appId);
        ...
        public PartialViewResult CreateNomogramData()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateNomogramData");
                }
                else
                {
        ...
        [HttpPost]
        public ActionResult InsertNomogramData(BusinessEntities.ConsentForms.NomogramData data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action)) 
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/Nomogram1");
                    ...
                    {
                        string file_path2 = Server.MapPath("~/images/ConsentForms/SavedImages/Nomogram2");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.NomogramData.isValidNomogramData(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.NomogramData.SaveNomogramData(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.ndn_appId, "NomogramData");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Nomogram Data Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateNomogramData(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.NomogramData ophtha = BusinessLogicLayer.ConsentForms.NomogramData.GetNomogramDataData(appId).FirstOrDefault();
                    return PartialView("UpdateNomogramData", ophtha);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public ActionResult EditNomogramData(BusinessEntities.ConsentForms.NomogramData data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.NomogramData.isValidNomogramData(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.NomogramData.SaveNomogramData(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Nomogram Data Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteNomogramData(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.NomogramData.DeleteNomogramData(appId, madeby);
        ...
        public PartialViewResult NomogramDataPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.NomogramData.GetNomogramDataPreviousHistory(appId);
                    return PartialView("NomogramDataPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlNomogramData(appId, emr, setId);
