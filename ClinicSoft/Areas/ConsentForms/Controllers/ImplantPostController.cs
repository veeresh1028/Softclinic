...
{
    public class ImplantPostController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;
        // GET: ConsentForms/ImplantPost
        public ActionResult ImplantPostIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetImplantPostData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConcentForms.ImplantPost> list = BusinessLogicLayer.ConcentForms.ImplantPost.GetImplantPostData(appId);
        ...
        public PartialViewResult CreateImplantPost()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateImplantPost");
                }
                else
                {
        ...
        public ActionResult InsertImplantPost(BusinessEntities.ConcentForms.ImplantPost data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.ImplantPost.isValidImplantPost(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConcentForms.ImplantPost.SaveImplantPost(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.ipp_appId, "ImplantPost");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Implant Post is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateImplantPost(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConcentForms.ImplantPost dental = BusinessLogicLayer.ConcentForms.ImplantPost.GetImplantPostData(appId).FirstOrDefault();

                    return PartialView("UpdateImplantPost", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditImplantPost(BusinessEntities.ConcentForms.ImplantPost data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.ImplantPost.isValidImplantPost(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConcentForms.ImplantPost.SaveImplantPost(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Implant Post is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public ActionResult DeleteImplantPost(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConcentForms.ImplantPost.DeleteImplantPost(appId, madeby);
        ...
        public PartialViewResult ImplantPostPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConcentForms.ImplantPost.GetImplantPostPreviousHistory(appId);
                    return PartialView("ImplantPostPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlImplantPost(appId, emr, setId);
