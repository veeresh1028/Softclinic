...
{
    public class AfterSurgeryController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;
        // GET: ConsentForms/AfterSurgery
        public ActionResult AfterSurgeryIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetAfterSurgeryData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.AfterSurgery> list = BusinessLogicLayer.ConsentForms.AfterSurgery.GetAfterSurgeryData(appId);
        ...
        public PartialViewResult CreateAfterSurgery()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateAfterSurgery");
                }
                else
                {
        ...
        public ActionResult InsertAfterSurgery(BusinessEntities.ConsentForms.AfterSurgery data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.AfterSurgery.isValidAfterSurgery(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.AfterSurgery.SaveAfterSurgery(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.sp_appId, "AfterSurgery");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "After Surgery is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateAfterSurgery(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.AfterSurgery dental = BusinessLogicLayer.ConsentForms.AfterSurgery.GetAfterSurgeryData(appId).FirstOrDefault();

                    return PartialView("UpdateAfterSurgery", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditAfterSurgery(BusinessEntities.ConsentForms.AfterSurgery data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.AfterSurgery.isValidAfterSurgery(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.AfterSurgery.SaveAfterSurgery(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "After Surgery is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        [HttpPost]
        public ActionResult DeleteAfterSurgery(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.AfterSurgery.DeleteAfterSurgery(appId, madeby);
        ...
        public PartialViewResult AfterSurgeryPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.AfterSurgery.GetAfterSurgeryPreviousHistory(appId);
                    return PartialView("AfterSurgeryPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlAfterSurgery(appId, emr, setId);
