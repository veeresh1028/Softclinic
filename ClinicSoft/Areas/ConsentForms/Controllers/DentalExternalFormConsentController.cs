...
{
    public class DentalExternalFormConsentController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;
        // GET: ConsentForms/DentalExternalFormConsent
        public ActionResult DentalExternalFormConsentIndex()
        {
            ViewBag.Tab = "0";
            return View();
        }
        public JsonResult GetDentalExternalFormConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.DentalExternalFormConsent> list = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.GetDentalExternalFormConsentData(appId);
        ...
        public PartialViewResult CreateDentalExternalFormConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateDentalExternalFormConsent");
                }
                else
                {
        ...
        public ActionResult InsertDentalExternalFormConsent(BusinessEntities.ConsentForms.DentalExternalFormConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/Tooth/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DentalExternalFormConsent.isValidDentalExternalFormConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.SaveDentalExternalFormConsent(data, madeby);

                        if (isInserted > 0)
                        {
                           //PDF _pdf = CreatePDF(data.cde_appId, "DentalExternalFormConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "DentalExternalFormConsent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDentalExternalFormConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.DentalExternalFormConsent ortho = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.GetDentalExternalFormConsentData(appId).FirstOrDefault();

                    return PartialView("UpdateDentalExternalFormConsent", ortho);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDentalExternalFormConsent(BusinessEntities.ConsentForms.DentalExternalFormConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DentalExternalFormConsent.isValidDentalExternalFormConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.SaveDentalExternalFormConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "DentalExternalFormConsent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDentalExternalFormConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.DeleteDentalExternalFormConsent(appId, madeby);
        ...
        public PartialViewResult DentalExternalFormConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DentalExternalFormConsent.GetDentalExternalFormConsentPreviousHistory(appId);
                    return PartialView("DentalExternalFormConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlDentalExternalFormConsent(appId, emr, setId);
