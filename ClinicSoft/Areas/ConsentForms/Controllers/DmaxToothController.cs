...
{
    public class DmaxToothController : Controller
    {
        int PageId = (int)Pages.DermatologyConsentForms;
        int PageId2 = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/DmaxTooth
        public ActionResult DmaxToothIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetDmaxToothData(int appId)
        {
            int Action = (int)Actions.Read;
            if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
            {
                List<BusinessEntities.ConsentForms.DmaxTooth> list = BusinessLogicLayer.ConsentForms.DmaxTooth.GetDmaxToothData(appId);
        ...
        public PartialViewResult CreateDmaxTooth()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    return PartialView("CreateDmaxTooth");
                }
                else
                {
        ...
        public ActionResult InsertDmaxTooth(BusinessEntities.ConsentForms.DmaxTooth data)
        {
            int isInserted = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxTooth.isValidDmaxTooth(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DmaxTooth.SaveDmaxTooth(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cdt_appId, "DmaxTooth");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Tooth Extraction Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDmaxTooth(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    BusinessEntities.ConsentForms.DmaxTooth derma = BusinessLogicLayer.ConsentForms.DmaxTooth.GetDmaxToothData(appId).FirstOrDefault();
                    return PartialView("UpdateDmaxTooth", derma);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDmaxTooth(BusinessEntities.ConsentForms.DmaxTooth data)
        {
            int isEdited = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxTooth.isValidDmaxTooth(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DmaxTooth.SaveDmaxTooth(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Tooth Extraction Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDmaxTooth(int appId)
        {
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    int val = BusinessLogicLayer.ConsentForms.DmaxTooth.DeleteDmaxTooth(appId, madeby);
        ...
        public PartialViewResult DmaxToothPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DmaxTooth.GetDmaxToothPreviousHistory(appId);
                    return PartialView("DmaxToothPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlDmaxTooth(appId, emr, setId);
