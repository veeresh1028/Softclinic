...
{
    public class ParsPlanaController : Controller
    {
        int PageId = (int)Pages.OphthalmologyConsentForms;
        // GET: ConsentForms/ParsPlana
        public ActionResult ParsPlanaIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetParsPlanaData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.ParsPlana> list = BusinessLogicLayer.ConsentForms.ParsPlana.GetParsPlanaData(appId);
        ...
        public PartialViewResult CreateParsPlana()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateParsPlana");
                }
                else
                {
        ...
        public ActionResult InsertParsPlana(BusinessEntities.ConsentForms.ParsPlana data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ParsPlana.isValidParsPlana(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.ParsPlana.SaveParsPlana(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.ppv_appId, "ParsPlana");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Pars Plana Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateParsPlana(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.ParsPlana ophtha = BusinessLogicLayer.ConsentForms.ParsPlana.GetParsPlanaData(appId).FirstOrDefault();
                    return PartialView("UpdateParsPlana", ophtha);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditParsPlana(BusinessEntities.ConsentForms.ParsPlana data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ParsPlana.isValidParsPlana(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.ParsPlana.SaveParsPlana(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Pars Plana Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteParsPlana(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.ParsPlana.DeleteParsPlana(appId, madeby);
        ...
        public PartialViewResult ParsPlanaPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.ParsPlana.GetParsPlanaPreviousHistory(appId);
                    return PartialView("ParsPlanaPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlParsPlana(appId, emr, setId);
