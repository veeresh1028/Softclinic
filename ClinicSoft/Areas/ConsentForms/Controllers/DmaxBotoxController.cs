...
{
    public class DmaxBotoxController : Controller
    {
        int PageId = (int)Pages.DermatologyConsentForms;
        int PageId2 = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/DmaxBotox
        public ActionResult DmaxBotoxIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetDmaxBotoxData(int appId)
        {
            int Action = (int)Actions.Read;
            if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
            {
                List<BusinessEntities.ConsentForms.DmaxBotox> list = BusinessLogicLayer.ConsentForms.DmaxBotox.GetDmaxBotoxData(appId);
        ...
        public PartialViewResult CreateDmaxBotox()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    return PartialView("CreateDmaxBotox");
                }
                else
                {
        ...
        public ActionResult InsertDmaxBotox(BusinessEntities.ConsentForms.DmaxBotox data)
        {
            int isInserted = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxBotox.isValidDmaxBotox(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DmaxBotox.SaveDmaxBotox(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cdb_appId, "DmaxBotox");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Botox Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDmaxBotox(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    BusinessEntities.ConsentForms.DmaxBotox derma = BusinessLogicLayer.ConsentForms.DmaxBotox.GetDmaxBotoxData(appId).FirstOrDefault();
                    return PartialView("UpdateDmaxBotox", derma);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDmaxBotox(BusinessEntities.ConsentForms.DmaxBotox data)
        {
            int isEdited = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxBotox.isValidDmaxBotox(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DmaxBotox.SaveDmaxBotox(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Botox Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDmaxBotox(int appId)
        {
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    int val = BusinessLogicLayer.ConsentForms.DmaxBotox.DeleteDmaxBotox(appId, madeby);
        ...
        public PartialViewResult DmaxBotoxPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DmaxBotox.GetDmaxBotoxPreviousHistory(appId);
                    return PartialView("DmaxBotoxPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlDmaxBotox(appId, emr, setId);
