...
{
    public class LaserSessionController : Controller
    {
        int PageId = (int)Pages.HijamaConsentForms;
        // GET: ConsentForms/LaserSession
        public ActionResult LaserSessionIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetLaserSessionData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.LaserSession> list = BusinessLogicLayer.ConsentForms.LaserSession.GetLaserSessionData(appId);
        ...
        public PartialViewResult CreateLaserSession()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateLaserSession");
                }
                else
                {
        ...
        public ActionResult InsertLaserSession(BusinessEntities.ConsentForms.LaserSession data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/Toplife1/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.LaserSession.isValidLaserSession(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.LaserSession.SaveLaserSession(data, madeby);

                        if (isInserted > 0)
                        {

                            PDF _pdf = CreatePDF(data.lsr_appId, "LaserSession");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Laser Session Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateLaserSession(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.LaserSession laser = BusinessLogicLayer.ConsentForms.LaserSession.GetLaserSessionData(appId).FirstOrDefault();

                    return PartialView("UpdateLaserSession", laser);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditLaserSession(BusinessEntities.ConsentForms.LaserSession data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.LaserSession.isValidLaserSession(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.LaserSession.SaveLaserSession(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Laser Session Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteLaserSession(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.LaserSession.DeleteLaserSession(appId, madeby);
        ...
        public PartialViewResult LaserSessionPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.LaserSession.GetLaserSessionPreviousHistory(appId);
                    return PartialView("LaserSessionPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlLaserSession(appId, emr, setId);
