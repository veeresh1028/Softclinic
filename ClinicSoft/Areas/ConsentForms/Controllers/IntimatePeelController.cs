...
{
    public class IntimatePeelController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/IntimatePeel
        public ActionResult IntimatePeelIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetIntimatePeelData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.IntimatePeel> list = BusinessLogicLayer.ConsentForms.IntimatePeel.GetIntimatePeelData(appId);
        ...
        public PartialViewResult CreateIntimatePeel()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateIntimatePeel");
                }
                else
                {
        ...
        public ActionResult InsertIntimatePeel(BusinessEntities.ConsentForms.IntimatePeel data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.IntimatePeel.isValidIntimatePeel(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.IntimatePeel.SaveIntimatePeel(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cipl_appId, "IntimatePeel");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Intimate Peel is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateIntimatePeel(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.IntimatePeel dental = BusinessLogicLayer.ConsentForms.IntimatePeel.GetIntimatePeelData(appId).FirstOrDefault();

                    return PartialView("UpdateIntimatePeel", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditIntimatePeel(BusinessEntities.ConsentForms.IntimatePeel data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.IntimatePeel.isValidIntimatePeel(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.IntimatePeel.SaveIntimatePeel(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Intimate Peel is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        [HttpPost]
        public ActionResult DeleteIntimatePeel(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.IntimatePeel.DeleteIntimatePeel(appId, madeby);
        ...
        public PartialViewResult IntimatePeelPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.IntimatePeel.GetIntimatePeelPreviousHistory(appId);
                    return PartialView("IntimatePeelPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlIntimatePeel(appId, emr, setId);
