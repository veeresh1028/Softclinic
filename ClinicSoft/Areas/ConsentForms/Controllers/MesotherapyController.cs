...
{
    public class MesotherapyController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/Mesotherapy
        public ActionResult MesotherapyIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetMesotherapyData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.Mesotherapy> list = BusinessLogicLayer.ConsentForms.Mesotherapy.GetMesotherapyData(appId);
        ...
        public PartialViewResult CreateMesotherapy()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateMesotherapy");
                }
                else
                {
        ...
        public ActionResult InsertMesotherapy(BusinessEntities.ConsentForms.Mesotherapy data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.Mesotherapy.isValidMesotherapy(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.Mesotherapy.SaveMesotherapy(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cmc_appId, "Mesotherapy");

                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Mesotherapy Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateMesotherapy(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.Mesotherapy meso = BusinessLogicLayer.ConsentForms.Mesotherapy.GetMesotherapyData(appId).FirstOrDefault();

                    return PartialView("UpdateMesotherapy", meso);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditMesotherapy(BusinessEntities.ConsentForms.Mesotherapy data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.Mesotherapy.isValidMesotherapy(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.Mesotherapy.SaveMesotherapy(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Mesotherapy Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteMesotherapy(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.Mesotherapy.DeleteMesotherapy(appId, madeby);
        ...
        public PartialViewResult MesotherapyPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.Mesotherapy.GetMesotherapyPreviousHistory(appId);
                    return PartialView("MesotherapyPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlMesotherapy(appId, emr, setId);
