...
{
    public class DmaxGeniusController : Controller
    {
        int PageId = (int)Pages.DermatologyConsentForms;
        int PageId2 = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/DmaxGenius
        public ActionResult DmaxGeniusIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetDmaxGeniusData(int appId)
        {
            int Action = (int)Actions.Read;
            if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
            {
                List<BusinessEntities.ConsentForms.DmaxGenius> list = BusinessLogicLayer.ConsentForms.DmaxGenius.GetDmaxGeniusData(appId);
        ...
        public PartialViewResult CreateDmaxGenius()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    return PartialView("CreateDmaxGenius");
                }
                else
                {
        ...
        public ActionResult InsertDmaxGenius(BusinessEntities.ConsentForms.DmaxGenius data)
        {
            int isInserted = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxGenius.isValidDmaxGenius(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DmaxGenius.SaveDmaxGenius(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cdg_appId, "DmaxGenius");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Genius Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDmaxGenius(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    BusinessEntities.ConsentForms.DmaxGenius derma = BusinessLogicLayer.ConsentForms.DmaxGenius.GetDmaxGeniusData(appId).FirstOrDefault();
                    return PartialView("UpdateDmaxGenius", derma);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDmaxGenius(BusinessEntities.ConsentForms.DmaxGenius data)
        {
            int isEdited = 0;
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxGenius.isValidDmaxGenius(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DmaxGenius.SaveDmaxGenius(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Genius Consent Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDmaxGenius(int appId)
        {
            try
            {
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    int val = BusinessLogicLayer.ConsentForms.DmaxGenius.DeleteDmaxGenius(appId, madeby);
        ...
        public PartialViewResult DmaxGeniusPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if ((PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action)))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DmaxGenius.GetDmaxGeniusPreviousHistory(appId);
                    return PartialView("DmaxGeniusPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlDmaxGenius(appId, emr, setId);
