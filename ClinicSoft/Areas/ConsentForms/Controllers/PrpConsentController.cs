...
{
    public class PrpConsentController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/PrpConsent
        public ActionResult PrpConsentIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetPrpConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.PrpConsent> list = BusinessLogicLayer.ConsentForms.PrpConsent.GetPrpConsentData(appId);
        ...
        public PartialViewResult CreatePrpConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreatePrpConsent");
                }
                else
                {
        ...
        public ActionResult InsertPrpConsent(BusinessEntities.ConsentForms.PrpConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PrpConsent.isValidPrpConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.PrpConsent.SavePrpConsent(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.prp_appId, "PrpConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Prp Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdatePrpConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.PrpConsent dental = BusinessLogicLayer.ConsentForms.PrpConsent.GetPrpConsentData(appId).FirstOrDefault();

                    return PartialView("UpdatePrpConsent", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditPrpConsent(BusinessEntities.ConsentForms.PrpConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PrpConsent.isValidPrpConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.PrpConsent.SavePrpConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Prp Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public ActionResult DeletePrpConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.PrpConsent.DeletePrpConsent(appId, madeby);
        ...
        public PartialViewResult PrpConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.PrpConsent.GetPrpConsentPreviousHistory(appId);
                    return PartialView("PrpConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlPrpConsent(appId, emr, setId);
