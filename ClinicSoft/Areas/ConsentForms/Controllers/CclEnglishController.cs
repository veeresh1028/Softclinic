...
{
    public class CclEnglishController : Controller
    {
        int PageId = (int)Pages.OphthalmologyConsentForms;
        // GET: ConsentForms/CclEnglish
        public ActionResult CclEnglishIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetCclEnglishData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.CclEnglish> list = BusinessLogicLayer.ConsentForms.CclEnglish.GetCclEnglishData(appId);
        ...
        public PartialViewResult CreateCclEnglish()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateCclEnglish");
                }
                else
                {
        ...
        public ActionResult InsertCclEnglish(BusinessEntities.ConsentForms.CclEnglish data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.CclEnglish.isValidCclEnglish(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.CclEnglish.SaveCclEnglish(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cef_appId, "CclEnglish");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Ccl English Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateCclEnglish(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.CclEnglish ophtha = BusinessLogicLayer.ConsentForms.CclEnglish.GetCclEnglishData(appId).FirstOrDefault();
                    return PartialView("UpdateCclEnglish", ophtha);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditCclEnglish(BusinessEntities.ConsentForms.CclEnglish data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.CclEnglish.isValidCclEnglish(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.CclEnglish.SaveCclEnglish(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Ccl English Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteCclEnglish(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.CclEnglish.DeleteCclEnglish(appId, madeby);
        ...
        public PartialViewResult CclEnglishPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.CclEnglish.GetCclEnglishPreviousHistory(appId);
                    return PartialView("CclEnglishPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlCclEnglish(appId, emr, setId);
