...
{
    public class SlipLampController : Controller
    {
        int PageId = (int)Pages.OphthalmologyConsentForms;
        // GET: ConsentForms/SlipLamp
        public ActionResult SlipLampIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetSlipLampData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.SlipLamp> list = BusinessLogicLayer.ConsentForms.SlipLamp.GetSlipLampData(appId);
        ...
        public PartialViewResult CreateSlipLamp()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateSlipLamp");
                }
                else
                {
        ...
        public ActionResult InsertSlipLamp(BusinessEntities.ConsentForms.SlipLamp data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.SlipLamp.isValidSlipLamp(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.SlipLamp.SaveSlipLamp(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.sln_appId, "SlipLamp");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Slip Lamp Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateSlipLamp(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.SlipLamp ophtha = BusinessLogicLayer.ConsentForms.SlipLamp.GetSlipLampData(appId).FirstOrDefault();
                    return PartialView("UpdateSlipLamp", ophtha);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditSlipLamp(BusinessEntities.ConsentForms.SlipLamp data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.SlipLamp.isValidSlipLamp(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.SlipLamp.SaveSlipLamp(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Slip Lamp Form is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteSlipLamp(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.SlipLamp.DeleteSlipLamp(appId, madeby);
        ...
        public PartialViewResult SlipLampPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.SlipLamp.GetSlipLampPreviousHistory(appId);
                    return PartialView("SlipLampPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlSlipLamp(appId, emr, setId);
