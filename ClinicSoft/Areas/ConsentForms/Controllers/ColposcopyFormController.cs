...
{
    public class ColposcopyFormController : Controller
    {
        int PageId = (int)Pages.GynaecologyConsentForms;
        // GET: ConsentForms/ColposcopyForm
        public ActionResult ColposcopyFormIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetColposcopyFormData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.ColposcopyForm> list = BusinessLogicLayer.ConsentForms.ColposcopyForm.GetColposcopyFormData(appId);
        ...
        public PartialViewResult CreateColposcopyForm()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateColposcopyForm");
                }
                else
                {
        ...
        public ActionResult InsertColposcopyForm(BusinessEntities.ConsentForms.ColposcopyForm data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/Colposcopy Form/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ColposcopyForm.isValidColposcopyForm(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.ColposcopyForm.SaveColposcopyForm(data, madeby);

                        if (isInserted > 0)
                        {

                             PDF _pdf = CreatePDF(data.ccf_appId, "ColposcopyForm");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "ColposcopyForm is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateColposcopyForm(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.ColposcopyForm colposcopy = BusinessLogicLayer.ConsentForms.ColposcopyForm.GetColposcopyFormData(appId).FirstOrDefault();

                    return PartialView("UpdateColposcopyForm", colposcopy);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditColposcopyForm(BusinessEntities.ConsentForms.ColposcopyForm data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.ColposcopyForm.isValidColposcopyForm(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.ColposcopyForm.SaveColposcopyForm(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "ColposcopyForm is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteColposcopyForm(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.ColposcopyForm.DeleteColposcopyForm(appId, madeby);
        ...
        public PartialViewResult ColposcopyFormPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.ColposcopyForm.GetColposcopyFormPreviousHistory(appId);
                    return PartialView("ColposcopyFormPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlColposcopyForm(appId, emr, setId);
