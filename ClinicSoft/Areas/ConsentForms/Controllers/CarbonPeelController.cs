...
{
    public class CarbonPeelController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/CarbonPeel
        public ActionResult CarbonPeelIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetCarbonPeelData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.CarbonPeel> list = BusinessLogicLayer.ConsentForms.CarbonPeel.GetCarbonPeelData(appId);
        ...
        public PartialViewResult CreateCarbonPeel()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateCarbonPeel");
                }
                else
                {
        ...
        public ActionResult InsertCarbonPeel(BusinessEntities.ConsentForms.CarbonPeel data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.CarbonPeel.isValidCarbonPeel(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.CarbonPeel.SaveCarbonPeel(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.cpc_appId, "CarbonPeel");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Carbon Peel is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateCarbonPeel(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.CarbonPeel dental = BusinessLogicLayer.ConsentForms.CarbonPeel.GetCarbonPeelData(appId).FirstOrDefault();

                    return PartialView("UpdateCarbonPeel", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditCarbonPeel(BusinessEntities.ConsentForms.CarbonPeel data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.CarbonPeel.isValidCarbonPeel(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.CarbonPeel.SaveCarbonPeel(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Carbon Peel is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public ActionResult DeleteCarbonPeel(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.CarbonPeel.DeleteCarbonPeel(appId, madeby);
        ...
        public PartialViewResult CarbonPeelPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.CarbonPeel.GetCarbonPeelPreviousHistory(appId);
                    return PartialView("CarbonPeelPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlCarbonPeel(appId, emr, setId);
