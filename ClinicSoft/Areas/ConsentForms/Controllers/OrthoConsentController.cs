...
{
    public class OrthoConsentController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;
        // GET: ConsentForms/OrthoConsent
        public ActionResult OrthoConsentIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetOrthoConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConcentForms.OrthoConsent> list = BusinessLogicLayer.ConcentForms.OrthoConsent.GetOrthoConsentData(appId);
        ...
        public PartialViewResult CreateOrthoConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateOrthoConsent");
                }
                else
                {
        ...
        public ActionResult InsertOrthoConsent(BusinessEntities.ConcentForms.OrthoConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.OrthoConsent.isValidOrthoConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConcentForms.OrthoConsent.SaveOrthoConsent(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.oc_appId, "OrthoConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Ortho Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateOrthoConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConcentForms.OrthoConsent dental = BusinessLogicLayer.ConcentForms.OrthoConsent.GetOrthoConsentData(appId).FirstOrDefault();

                    return PartialView("UpdateOrthoConsent", dental);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditOrthoConsent(BusinessEntities.ConcentForms.OrthoConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConcentForms.OrthoConsent.isValidOrthoConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConcentForms.OrthoConsent.SaveOrthoConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Ortho Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        [HttpPost]
        public ActionResult DeleteOrthoConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConcentForms.OrthoConsent.DeleteOrthoConsent(appId, madeby);
        ...
        public PartialViewResult OrthoConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConcentForms.OrthoConsent.GetOrthoConsentPreviousHistory(appId);
                    return PartialView("OrthoConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConcentFormPDF.HtmlOrthoConsent(appId, emr, setId);
