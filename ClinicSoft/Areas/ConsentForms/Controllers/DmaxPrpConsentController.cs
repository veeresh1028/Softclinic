...
{
    public class DmaxPrpConsentController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        int PageId2 = (int)Pages.DermatologyConsentForms;
        // GET: ConsentForms/DmaxPrpConsent
        public ActionResult DmaxPrpConsentIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetDmaxPrpConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
            {
                List<BusinessEntities.ConsentForms.DmaxPrpConsent> list = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.GetDmaxPrpConsentData(appId);
        ...
        public PartialViewResult CreateDmaxPrpConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    return PartialView("CreateDmaxPrpConsent");
                }
                else
                {
        ...
        public ActionResult InsertDmaxPrpConsent(BusinessEntities.ConsentForms.DmaxPrpConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxPrpConsent.isValidDmaxPrpConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.SaveDmaxPrpConsent(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.dpc_appId, "DmaxPrpConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "DmaxPrpConsent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateDmaxPrpConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    BusinessEntities.ConsentForms.DmaxPrpConsent dmax = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.GetDmaxPrpConsentData(appId).FirstOrDefault();
                    return PartialView("UpdateDmaxPrpConsent", dmax);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditDmaxPrpConsent(BusinessEntities.ConsentForms.DmaxPrpConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.DmaxPrpConsent.isValidDmaxPrpConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.SaveDmaxPrpConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "DmaxPrpConsent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteDmaxPrpConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.DeleteDmaxPrpConsent(appId, madeby);
        ...
        public PartialViewResult DmaxPrpConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action) || PageControl.haveAccess(PageId2, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistory> list = BusinessLogicLayer.ConsentForms.DmaxPrpConsent.GetDmaxPrpConsentPreviousHistory(appId);
                    return PartialView("DmaxPrpConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlDmaxPrpConsent(appId, emr, setId);
