...
{
    public class EmatrixController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;
        // GET: ConsentForms/Ematrix
        public ActionResult EmatrixIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetEmatrixData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.Ematrix> list = BusinessLogicLayer.ConsentForms.Ematrix.GetEmatrixData(appId);
        ...
        public PartialViewResult CreateEmatrix()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateEmatrix");
                }
                else
                {
        ...
        public ActionResult InsertEmatrix(BusinessEntities.ConsentForms.Ematrix data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.Ematrix.isValidEmatrix(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.Ematrix.SaveEmatrix(data, madeby);

                        if (isInserted > 0)
                        {

                            //PDF _pdf = CreatePDF(data.ecf_appId, "Ematrix");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Ematrix Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateEmatrix(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.Ematrix ematrix = BusinessLogicLayer.ConsentForms.Ematrix.GetEmatrixData(appId).FirstOrDefault();

                    return PartialView("UpdateEmatrix", ematrix);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditEmatrix(BusinessEntities.ConsentForms.Ematrix data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.Ematrix.isValidEmatrix(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.Ematrix.SaveEmatrix(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Ematrix Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteEmatrix(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.Ematrix.DeleteEmatrix(appId, madeby);
        ...
        public PartialViewResult EmatrixPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.Ematrix.GetEmatrixPreviousHistory(appId);
                    return PartialView("EmatrixPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlEmatrix(appId, emr, setId);
