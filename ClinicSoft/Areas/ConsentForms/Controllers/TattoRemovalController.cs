...
{
    public class TattoRemovalController : Controller
    {
        int PageId = (int)Pages.LaserConsentForms;

        // GET: ConsentForms/TattoRemoval
        public ActionResult TattoRemovalIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetTattoRemovalData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.TattoRemoval> list = BusinessLogicLayer.ConsentForms.TattoRemoval.GetTattoRemovalData(appId);
        ...
        public PartialViewResult CreateTattoRemoval()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateTattoRemoval");
                }
                else
                {
        ...
        public ActionResult InsertTattoRemoval(BusinessEntities.ConsentForms.TattoRemoval data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.TattoRemoval.isValidTattoRemoval(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.TattoRemoval.SaveTattoRemoval(data, madeby);

                        if (isInserted > 0)
                        {

                            PDF _pdf = CreatePDF(data.trr_appId, "TattoRemoval");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Tatto Removal Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateTattoRemoval(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.TattoRemoval tatto = BusinessLogicLayer.ConsentForms.TattoRemoval.GetTattoRemovalData(appId).FirstOrDefault();

                    return PartialView("UpdateTattoRemoval", tatto);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditTattoRemoval(BusinessEntities.ConsentForms.TattoRemoval data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.TattoRemoval.isValidTattoRemoval(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.TattoRemoval.SaveTattoRemoval(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Tatto Removal Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteTattoRemoval(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.TattoRemoval.DeleteTattoRemoval(appId, madeby);
        ...
        public PartialViewResult TattoRemovalPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.TattoRemoval.GetTattoRemovalPreviousHistory(appId);
                    return PartialView("TattoRemovalPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlTattoRemoval(appId, emr, setId);
