...
{
    public class PhysicalExamFormController : Controller
    {
        int PageId = (int)Pages.GynaecologyConsentForms;
        // GET: ConsentForms/PhysicalExamForm
        public ActionResult PhysicalExamFormIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetPhysicalExamFormData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.PhysicalExamForm> list = BusinessLogicLayer.ConsentForms.PhysicalExamForm.GetPhysicalExamFormData(appId);
        ...
        public PartialViewResult CreatePhysicalExamForm()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreatePhysicalExamForm");
                }
                else
                {
        ...
        public ActionResult InsertPhysicalExamForm(BusinessEntities.ConsentForms.PhysicalExamForm data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/PhysicalExamForm/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PhysicalExamForm.isValidPhysicalExamForm(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.PhysicalExamForm.SavePhysicalExamForm(data, madeby);

                        if (isInserted > 0)
                        {

                             PDF _pdf = CreatePDF(data.pef_appId, "PhysicalExamForm");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "PhysicalExamForm is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdatePhysicalExamForm(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.PhysicalExamForm physical = BusinessLogicLayer.ConsentForms.PhysicalExamForm.GetPhysicalExamFormData(appId).FirstOrDefault();

                    return PartialView("UpdatePhysicalExamForm", physical);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditPhysicalExamForm(BusinessEntities.ConsentForms.PhysicalExamForm data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PhysicalExamForm.isValidPhysicalExamForm(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.PhysicalExamForm.SavePhysicalExamForm(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "PhysicalExamForm is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeletePhysicalExamForm(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.PhysicalExamForm.DeletePhysicalExamForm(appId, madeby);
        ...
        public PartialViewResult PhysicalExamFormPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.PhysicalExamForm.GetPhysicalExamFormPreviousHistory(appId);
                    return PartialView("PhysicalExamFormPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlPhysicalExamForm(appId, emr, setId);
