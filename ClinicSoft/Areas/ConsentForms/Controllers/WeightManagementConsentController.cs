...
{
    public class WeightManagementConsentController : Controller
    {
        int PageId = (int)Pages.HijamaConsentForms;
        // GET: ConsentForms/WeightManagementConsent
        public ActionResult WeightManagementConsentIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetWeightManagementConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.WeightManagementConsent> list = BusinessLogicLayer.ConsentForms.WeightManagementConsent.GetWeightManagementConsentData(appId);
        ...
        public PartialViewResult CreateWeightManagementConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateWeightManagementConsent");
                }
                else
                {
        ...
        public ActionResult InsertWeightManagementConsent(BusinessEntities.ConsentForms.WeightManagementConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/WeightConsent/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.WeightManagementConsent.isValidWeightManagementConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.WeightManagementConsent.SaveWeightManagementConsent(data, madeby);

                        if (isInserted > 0)
                        {

                            PDF _pdf = CreatePDF(data.wmc_appId, "WeightManagementConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Weight Management Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateWeightManagementConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.WeightManagementConsent weightmanagement = BusinessLogicLayer.ConsentForms.WeightManagementConsent.GetWeightManagementConsentData(appId).FirstOrDefault();

                    return PartialView("UpdateWeightManagementConsent", weightmanagement);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditWeightManagementConsent(BusinessEntities.ConsentForms.WeightManagementConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.WeightManagementConsent.isValidWeightManagementConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.WeightManagementConsent.SaveWeightManagementConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Weight Management Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteWeightManagementConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.WeightManagementConsent.DeleteWeightManagementConsent(appId, madeby);
        ...
        public PartialViewResult WeightManagementConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.WeightManagementConsent.GetWeightManagementConsentPreviousHistory(appId);
                    return PartialView("WeightManagementConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlWeightManagementConsent(appId, emr, setId);
