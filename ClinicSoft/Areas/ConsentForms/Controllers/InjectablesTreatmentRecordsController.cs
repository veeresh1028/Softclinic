...
{
    public class InjectablesTreatmentRecordsController : Controller
    {
        int PageId = (int)Pages.DermatologyConsentForms;
        // GET: ConsentForms/InjectablesTreatmentRecords
        public ActionResult InjectablesTreatmentRecordsIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetInjectablesTreatmentRecordsData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.InjectablesTreatmentRecords> list = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.GetInjectablesTreatmentRecordsData(appId);
        ...
        public PartialViewResult CreateInjectablesTreatmentRecords()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateInjectablesTreatmentRecords");
                }
                else
                {
        ...
        public ActionResult InsertInjectablesTreatmentRecords(BusinessEntities.ConsentForms.InjectablesTreatmentRecords data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("/images/ConsentForms/SavedImages/InjectablesTreatmentRecords/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.InjectablesTreatmentRecords.isValidInjectablesTreatmentRecords(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.SaveInjectablesTreatmentRecords(data, madeby);

                        if (isInserted > 0)
                        {

                             PDF _pdf = CreatePDF(data.itr_appId, "InjectablesTreatmentRecords");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "InjectablesTreatmentRecords is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateInjectablesTreatmentRecords(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.InjectablesTreatmentRecords injectables = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.GetInjectablesTreatmentRecordsData(appId).FirstOrDefault();

                    return PartialView("UpdateInjectablesTreatmentRecords", injectables);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditInjectablesTreatmentRecords(BusinessEntities.ConsentForms.InjectablesTreatmentRecords data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.InjectablesTreatmentRecords.isValidInjectablesTreatmentRecords(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.SaveInjectablesTreatmentRecords(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "InjectablesTreatmentRecords is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteInjectablesTreatmentRecords(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.DeleteInjectablesTreatmentRecords(appId, madeby);
        ...
        public PartialViewResult InjectablesTreatmentRecordsPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.InjectablesTreatmentRecords.GetInjectablesTreatmentRecordsPreviousHistory(appId);
                    return PartialView("InjectablesTreatmentRecordsPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlInjectablesTreatmentRecords(appId, emr, setId);
