...
{
    public class HijjamaConsentController : Controller
    {
        int PageId = (int)Pages.HijamaConsentForms;
        // GET: ConsentForms/HijjamaConsent
        public ActionResult HijjamaConsentIndex()
        {
            ViewBag.Tab = "0";

            return View();
        }
        public JsonResult GetHijjamaConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConsentForms.HijjamaConsent> list = BusinessLogicLayer.ConsentForms.HijjamaConsent.GetHijjamaConsentData(appId);
        ...
        public PartialViewResult CreateHijjamaConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreateHijjamaConsent");
                }
                else
                {
        ...
        public ActionResult InsertHijjamaConsent(BusinessEntities.ConsentForms.HijjamaConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    {
                        string file_path = Server.MapPath("~/images/ConsentForms/SavedImages/Hijjama/");
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.HijjamaConsent.isValidHijjamaConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConsentForms.HijjamaConsent.SaveHijjamaConsent(data, madeby);

                        if (isInserted > 0)
                        {

                            PDF _pdf = CreatePDF(data.hcf_appId, "HijjamaConsent");
                            return Json(new { isInserted = true, isSuccess = true, message = "Done" }, JsonRequestBehavior.AllowGet);
                        }
                        else
                        {
                            return Json(new { isInserted = false, isSuccess = true, message = "Hijjama Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        public PartialViewResult UpdateHijjamaConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConsentForms.HijjamaConsent hijjama = BusinessLogicLayer.ConsentForms.HijjamaConsent.GetHijjamaConsentData(appId).FirstOrDefault();

                    return PartialView("UpdateHijjamaConsent", hijjama);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditHijjamaConsent(BusinessEntities.ConsentForms.HijjamaConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.HijjamaConsent.isValidHijjamaConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConsentForms.HijjamaConsent.SaveHijjamaConsent(data, madeby);
                        ...
                        {
                            return Json(new { isEdited = false, isSuccess = true, message = "Hijjama Consent is already exists!" }, JsonRequestBehavior.AllowGet);
                        }
                    }
                    else
                    {
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeleteHijjamaConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConsentForms.HijjamaConsent.DeleteHijjamaConsent(appId, madeby);
        ...
        public PartialViewResult HijjamaConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConsentForms.HijjamaConsent.GetHijjamaConsentPreviousHistory(appId);
                    return PartialView("HijjamaConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlHijjamaConsent(appId, emr, setId);
