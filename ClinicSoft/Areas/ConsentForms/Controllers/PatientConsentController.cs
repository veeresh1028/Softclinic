...
{
    public class PatientConsentController : Controller
    {
        int PageId = (int)Pages.DentalConsentForms;

        // GET: ConsentForms/PatientConsent
        public ActionResult PatientConsentIndex()
        {
            ViewBag.Tab = "0";
        ...
        public JsonResult GetPatientConsentData(int appId)
        {
            int Action = (int)Actions.Read;

            if (PageControl.haveAccess(PageId, Action))
            {
                List<BusinessEntities.ConcentForms.PatientConsent> list = BusinessLogicLayer.ConcentForms.PatientConsent.GetPatientConsentData(appId);
        ...
        public PartialViewResult CreatePatientConsent()
        {
            if (PageControl.getLoggedinId() > 0)
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    return PartialView("CreatePatientConsent");
                }
                else
                {
        ...
        public ActionResult InsertPatientConsent(BusinessEntities.ConcentForms.PatientConsent data)
        {
            int isInserted = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PatientConsent.isValidPatientConsent(data, out errors))
                    {
                        isInserted = BusinessLogicLayer.ConcentForms.PatientConsent.SavePatientConsent(data, madeby);

                        if (isInserted > 0)
                        {
                            PDF _pdf = CreatePDF(data.pc_appId, "PatientConsent");
        ...
        public PartialViewResult UpdatePatientConsent(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Create;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    BusinessEntities.ConcentForms.PatientConsent patient = BusinessLogicLayer.ConcentForms.PatientConsent.GetPatientConsentData(appId).FirstOrDefault();

                    return PartialView("UpdatePatientConsent", patient);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditPatientConsent(BusinessEntities.ConcentForms.PatientConsent data)
        {
            int isEdited = 0;
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    ...
                    if (SecurityLayer.FormValidations.ConsentForms.PatientConsent.isValidPatientConsent(data, out errors))
                    {
                        isEdited = BusinessLogicLayer.ConcentForms.PatientConsent.SavePatientConsent(data, madeby);
        ...
        // Delete 
        [HttpPost]
        public ActionResult DeletePatientConsent(int appId)
        {
            try
            {
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    int val = BusinessLogicLayer.ConcentForms.PatientConsent.DeletePatientConsent(appId, madeby);
        ...
        public PartialViewResult PatientConsentPreviousHistory(int appId)
        {
            if (PageControl.getLoggedinId() > 0)
            {
                int Action = (int)Actions.Read;
                ...
                if (PageControl.haveAccess(PageId, Action))
                {
                    List<BusinessEntities.ConcentForms.ConcentPreviousHistroy> list = BusinessLogicLayer.ConcentForms.PatientConsent.GetPatientConsentPreviousHistory(appId);
                    return PartialView("PatientConsentPreviousHistory", list);
                }
                else
                {
                    return PartialView("_Unauthorized");
        ...
        public PDF CreatePDF(int appId, string form)
        {
            ...
            try
            {
                ...
                string htmlContent = BusinessLogicLayer.ConcentForms.ConsentFormPDF.HtmlPatientConsent(appId, emr, setId);
