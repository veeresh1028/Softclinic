...
{
    public class DentalPatientTreatmentByNationality
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.DentalPatientTreatmentByNationality> SearchDentalPatientTreatmentReport(DentalPatientTreatmentReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.DentalPatientTreatmentByNationality> reports = new List<BusinessEntities.Appointment.DHA_Reports.DentalPatientTreatmentByNationality>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.DentalPatientTreatmentByNationality.SearchLaboratoryTestsByNationality(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.DentalPatientTreatmentByNationality report = new BusinessEntities.Appointment.DHA_Reports.DentalPatientTreatmentByNationality();
