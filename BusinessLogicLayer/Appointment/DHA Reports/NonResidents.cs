...
{
    public class NonResidents
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.NonResidents> SearchNonResidentsReport(NonResidentsReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.NonResidents> reports = new List<BusinessEntities.Appointment.DHA_Reports.NonResidents>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.NonResidents.SearchNonResidentsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.NonResidents report = new BusinessEntities.Appointment.DHA_Reports.NonResidents();
                        report.nationalities = dr["nationality"].ToString();
