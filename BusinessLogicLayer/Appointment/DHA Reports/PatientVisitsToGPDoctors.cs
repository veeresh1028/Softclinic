...
{
    public class PatientVisitsToGPDoctors
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToGPDoctors> SearchPatientVisitsToGPDoctors(PatientVisitsToGPDoctorsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToGPDoctors> reports = new List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToGPDoctors>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.PatientVisitsToGPDoctors.SearchPatientVisitsToGPDoctors(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.PatientVisitsToGPDoctors report = new BusinessEntities.Appointment.DHA_Reports.PatientVisitsToGPDoctors();
