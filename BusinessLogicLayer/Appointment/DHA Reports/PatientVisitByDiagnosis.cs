...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.PatientVisitByDiagnosis> SearchPatientVisitByDiagnosis(PatientVisitReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.PatientVisitByDiagnosis> reports = new List<BusinessEntities.Appointment.DHA_Reports.PatientVisitByDiagnosis>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.PatientVisitByDiagnosis.SearchPatientVisitByDiagnosis(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.PatientVisitByDiagnosis report = new BusinessEntities.Appointment.DHA_Reports.PatientVisitByDiagnosis();
