...
{
    public class RadiologyExaminations
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminations> SearchRadiologyExaminationsReport(RadiologyExaminationsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminations> reports = new List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminations>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.RadiologyExaminations.SearchRadiologyExaminationsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.RadiologyExaminations report = new BusinessEntities.Appointment.DHA_Reports.RadiologyExaminations();
