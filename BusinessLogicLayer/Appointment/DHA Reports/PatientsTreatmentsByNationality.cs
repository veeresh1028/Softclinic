...
{
    public class PatientsTreatmentsByNationality
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.PatientsTreatmentsByNationality> SearchPatientsByNationalityTreatments(PatientsTreatmentsReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.PatientsTreatmentsByNationality> reports = new List<BusinessEntities.Appointment.DHA_Reports.PatientsTreatmentsByNationality>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.PatientsTreatmentsByNationality.SearchPatientsByNationalityTreatments(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.PatientsTreatmentsByNationality report = new BusinessEntities.Appointment.DHA_Reports.PatientsTreatmentsByNationality();
                        report.tr_name = dr["tr_name"].ToString();
                        report.num_treatment = Convert.ToInt32(dr["num_treatment"]);
