...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.LaboratoryTests> SearchLaboratoryTestsReport(LaboratoryTestsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.LaboratoryTests> reports = new List<BusinessEntities.Appointment.DHA_Reports.LaboratoryTests>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.LaboratoryTests.SearchLaboratoryTestsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.LaboratoryTests report = new BusinessEntities.Appointment.DHA_Reports.LaboratoryTests();
