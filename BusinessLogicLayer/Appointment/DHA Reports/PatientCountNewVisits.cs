...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.PatientCountNewVisits> SearchPatientCountNewVisits(PatientCountNewVisitsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.PatientCountNewVisits> reports = new List<BusinessEntities.Appointment.DHA_Reports.PatientCountNewVisits>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.PatientCountNewVisits.SearchPatientCountNewVisits(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.PatientCountNewVisits report = new BusinessEntities.Appointment.DHA_Reports.PatientCountNewVisits();
                        ...
                        report.M_4_9 = Convert.ToInt32(dr["dpn_5_9M"]);
                        report.F_4_9 = Convert.ToInt32(dr["dpn_5_9F"]);
