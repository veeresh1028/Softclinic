...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.ResidentsUAENationals> SearchResidentsUAENationalsReport(ResidentsReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.ResidentsUAENationals> reports = new List<BusinessEntities.Appointment.DHA_Reports.ResidentsUAENationals>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.ResidentsUAENationals.SearchResidentsUAENationalsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.ResidentsUAENationals report = new BusinessEntities.Appointment.DHA_Reports.ResidentsUAENationals();
                        report.nationalities = dr["pat_city"].ToString();
