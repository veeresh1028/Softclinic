...
{
    public class DentalPatientsAndTreatments
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsAndTreatments> SearchDentalPatientsAndTreatmentsReport(DentalPatientsAndTreatmentsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsAndTreatments> reports = new List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsAndTreatments>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.DentalPatientsAndTreatments.SearchDentalPatientsAndTreatmentsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.DentalPatientsAndTreatments report = new BusinessEntities.Appointment.DHA_Reports.DentalPatientsAndTreatments();
