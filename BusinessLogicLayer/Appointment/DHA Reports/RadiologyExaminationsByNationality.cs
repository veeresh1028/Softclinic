...
{
    public class RadiologyExaminationsByNationality
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminationsByNationality> SearchRadiologyExaminationsReport(RadiologyExaminationsReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminationsByNationality> reports = new List<BusinessEntities.Appointment.DHA_Reports.RadiologyExaminationsByNationality>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.RadiologyExaminationsByNationality.SearchRadiologyExaminationsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.RadiologyExaminationsByNationality report = new BusinessEntities.Appointment.DHA_Reports.RadiologyExaminationsByNationality();
