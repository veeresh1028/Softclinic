...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality> SearchDentalPatientsTreatmentsReport(DentalPatientsTreatmentsReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality> reports = new List<BusinessEntities.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality.SearchDentalPatientsTreatmentsReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality report = new BusinessEntities.Appointment.DHA_Reports.DentalPatientsTreatmentsByNationality();
