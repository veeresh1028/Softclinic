...
{
    public class OutPatientsRehabilitation
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.OutPatientsRehabilitation> SearchOutPatientsRehabilitationReport(OutPatientsRehabilitationReportSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.OutPatientsRehabilitation> reports = new List<BusinessEntities.Appointment.DHA_Reports.OutPatientsRehabilitation>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.OutPatientsRehabilitation.SearchOutPatientsRehabilitationReport(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.OutPatientsRehabilitation report = new BusinessEntities.Appointment.DHA_Reports.OutPatientsRehabilitation();
