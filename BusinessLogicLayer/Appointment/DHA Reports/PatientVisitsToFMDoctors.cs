...
namespace BusinessLogicLayer.Appointment.DHA_Reports
{
    ...
    {
        public static List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToFMDoctors> SearchPatientVisitsToFMDoctors(PatientVisitsToFMDoctorsSearch search)
        {
            try
            {
                List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToFMDoctors> reports = new List<BusinessEntities.Appointment.DHA_Reports.PatientVisitsToFMDoctors>();

                DataTable dt = DataAccessLayer.Appointment.DHA_Reports.PatientVisitsToFMDoctors.SearchPatientVisitsToFMDoctors(search);

                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        BusinessEntities.Appointment.DHA_Reports.PatientVisitsToFMDoctors report = new BusinessEntities.Appointment.DHA_Reports.PatientVisitsToFMDoctors();
